#!/usr/bin/env bash
# Entrypoint kept for backward compatibility — delegates to scripts/deploy.sh
...
#
# Usage: ./deploy.sh [--skip-prereqs] [--skip-transfer] [--no-test]
# =============================================================================
...
if [ "${SKIP_PREREQS}" = true ]; then
    ...
    bash "${SCRIPT_DIR}/manifests/00-prerequisites-check.sh" || \
...
oc apply -f "${SCRIPT_DIR}/manifests/01-namespace.yaml"
...
if [ "${MINIO_READY:-0}" -ge 1 ] 2>/dev/null; then
    ...
    oc apply -f "${SCRIPT_DIR}/manifests/03-minio.yaml"
...
# Bucket creation is handled by the model transfer Job (step [2/4] runs aws s3 mb idempotently)
...
envsubst '${NAMESPACE} ${HF_TOKEN}' \
    < "${SCRIPT_DIR}/manifests/04-hf-secret.yaml.template" | oc apply -f -
...
envsubst '${NAMESPACE} ${MINIO_ACCESS_KEY} ${MINIO_SECRET_KEY} ${MINIO_ENDPOINT} ${MINIO_BUCKET}' \
    < "${SCRIPT_DIR}/manifests/05-s3-connection.yaml.template" | oc apply -f -
log_success "Secret 's3-data-connection' applied"

envsubst '${NAMESPACE} ${MINIO_ACCESS_KEY} ${MINIO_SECRET_KEY} ${MINIO_ENDPOINT} ${MINIO_BUCKET}' \
    < "${SCRIPT_DIR}/manifests/10-kserve-model-sa.yaml.template" | oc apply -f -
...
oc apply -f "${SCRIPT_DIR}/manifests/07-accelerator-profile.yaml" 2>/dev/null || \
...
if [ "${SKIP_TRANSFER}" = true ]; then
    ...
    # Check if model already in MinIO
    MODEL_EXISTS=false
    MINIO_POD=$(oc get pod -n "${MINIO_NAMESPACE}" -l app=minio \
        -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")
    ...
    if [ "${MODEL_EXISTS}" = true ]; then
        ...
        envsubst '${NAMESPACE}' \
            < "${SCRIPT_DIR}/manifests/06-model-transfer-job.yaml.template" | oc apply -f -
...
# Remove old custom runtime if it exists from a previous run
...
# Use the RHOAI-managed vllm-cuda-runtime-template. This is the same runtime
# the RHOAI Dashboard uses, so it's recognized and shown correctly in the UI.
...
# Add Dashboard recognition annotations so the RHOAI UI shows the runtime name
# rather than 'Unknown Serving Runtime'.
...
# Patch args to add time-slicing constraints. The template default args are:
#   [--port=8080, --model=/mnt/models, --served-model-name={{.Name}}]
# We append GPU memory and context length limits so 7 instances coexist on
# a single time-sliced T4 without OOM-killing each other.
...
# --no-enable-chunked-prefill: Triton unified_attention kernel needs 80 KB shared memory,
#   which exceeds T4 (Turing) 64 KB hardware limit. Disabling chunked prefill avoids it.
# --enforce-eager: remaining Triton CUDA-graph profiling kernels also exceed T4 shared memory
#   limit. Eager mode skips CUDA graph capture entirely.
# These two args together allow vLLM 0.18 to start on T4 GPUs.
...
oc apply -f "${SCRIPT_DIR}/manifests/09-inference-service.yaml"
...
log_info "Monitor: oc get inferenceservice gemma-270m -n ${NAMESPACE} -w"
echo ""
...
envsubst '${NAMESPACE} ${INFERENCE_ENDPOINT}' \
    < "${SCRIPT_DIR}/manifests/11-playground.yaml.template" | oc apply -f -
...
if [ "${NO_TEST}" = false ]; then
    ...
    case "${RUN_TESTS:-Y}" in
        [Yy]* | "")
            bash "${SCRIPT_DIR}/manifests/12-test-inference.sh"
            ;;
        *)
